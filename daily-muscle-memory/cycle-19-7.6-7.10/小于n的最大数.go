...
/*
思路重复：
大体思路是，从 n 的高位开始，一位一位的看，能使用 digits 中的什么数字，然后最后根据每一位的选择，来构建出最终的 res。
那么如何选每一位呢？除开最后一位，每一位都优先看看能否使用相同的数字，如果没有，我们选择小于当前位置的最大数。
并且一旦选到了，后面就没必要继续选择了，直接返回去构建结果。当构建结果的时候，如果对应位有数，增加对应的值，
如果没有，则说明前面已经决定好了，直接使用 digits 的最大值来构建 res。

但是如果是最后一位，就只能挑选小于它的最大数，否则可能会越界。然后只需要查看小于它的最大数即可，如果都没有，就得回溯前面的去看看。重新挑选。
回溯也是一样，从当前位置开始回溯，一直回溯到数的最后一位，如果在中间某一位找到了比它更小的最大数，那么就可以退出所有循环了。
